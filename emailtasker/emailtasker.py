...
class Tasker(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if user_response == self.config['monitor']['exit_kw']: break
            elif user_response == self.config['monitor']['restart_kw']: continue

        return

    def self.get_admin_response(self):
        ...
        while (datetime.datetime.now() - start).total_seconds() < wait:
            ...
            if len(ids[0].split()) > 0:
                ...
                self.last_received_reply_id = response.get("Message-ID")
    ...
    def _format_message(self,
        ...
        else if message_body_html is not None:
